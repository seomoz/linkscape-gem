...
This returns a Linkscape::Response::ResponseData object, which is the container class for the response data.  From there, you can request specific data points, like:
...
Example: <tt>client.mozRank(uri)</tt>

=== urlMetrics

Returns metrics about the supplied URI.

Example: <tt>client.urlMetrics(uri, :cols => :all)</tt>
...
Example: <tt>client.topLinks(uri, :page, :urlcols => :all, :linkcols => :all, :limit => 3))</tt>
...
Example: <tt>client.allLinks(uri, :page, :urlcols => [:title, :url, :page_authority, :domain_authority], :linkcols => :all, :filters => :external, :limit => 3)</tt>
...
* <tt>:targetcols</tt> - An array of data columns (see below) that should be returned for the link target.
* <tt>:linkcols</tt> - An array of data columns (see below) that should be returned for the link itself.
* <tt>:filters</tt> - A String, Hash, Array or Sybol of filters (see below) that should be applied to the list of links.
...
Example: <tt>client.topPages(uri, :page, :cols => :all, :limit => 3, :limit => 3)</tt>
...
Example: <tt>client.anchorMetrics(uri, :cols => :all, :scope => "page_to_domain", :filters => :external, :sort => :domains_linking_page, :limit => 3, :scope => :phrase_to_page)</tt>
...
* <tt>:filters</tt> - A symbol representing the 'filter' of the anchor text data (see below).
...
The following columns are available for requesting from the API.
...
Links may be filtered to only include the following.
